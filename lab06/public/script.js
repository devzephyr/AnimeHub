...
(function () {
  ...
  const defaultPasswordHelp = passwordHelp ? passwordHelp.textContent : '';
  ...
  function validatePassword() {
    ...
    if (passwordHelp) {
      passwordHelp.textContent = s ? s : defaultPasswordHelp;
    }
  ...
  form.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      try {
        ...
        // If the body is empty or not JSON, log and continue with null data.
        console.error('Failed to parse server response as JSON:', jsonErr);
      }

      if (!response.ok || !data) {
        const errorMessage = (data && data.error) || 'Server error. Please try again later.';
        setFormStatus('error', errorMessage);
        return;
      }

      if (!data.ok) {
        setFormStatus('error', data.error || 'Server validation failed.');
        return;
      }

      setFormStatus('success', data.message || 'Welcome to the Fellowship!');
      form.reset();
      [nameError, emailError, passwordError, ageError, classError].forEach((el) => {
        if (el) el.textContent = '';
      });
      [nameInput, emailInput, passwordInput, ageInput, classSelect].forEach((el) => el.removeAttribute('aria-invalid'));
      if (passwordHelp) {
        passwordHelp.textContent = defaultPasswordHelp;
      }
    } catch (err) {
